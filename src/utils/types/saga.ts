...
type StripEffects<T> = T extends IterableIterator<infer E>
  ? E extends Effect | SimpleEffect<never>
    ? never
    : E
  : never
...
type DecideReturn<T> = T extends Promise<infer R>
  ...
  : T extends IterableIterator<never>
 ...
 */
export type CallReturnType<T extends (...args: Array<never>) => any> = DecideReturn<ReturnType<T>>

/** Get the return type of a saga, stripped of any effects the saga might yield, which will be handled by Saga. */
export type SagaReturnType<T extends (...args: Array<never>) => never> = StripEffects<ReturnType<T>>
